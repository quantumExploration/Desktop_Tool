...
class svPrimitive
{
 public:
  ...
  svPrimitive(svQDOTData* f, svParticle* sd, 
              svInt sd_num, svUint mode);
	svPrimitive(svQDOTData *f){field = f;}
  virtual ~svPrimitive();
  ...
  svQDOTData* GetField() const
	{  return field;  }

  void SetField(svQDOTData* f)
	{  field = f;  }
 ...
 protected:
  svQDOTData* field;// the field to advect
