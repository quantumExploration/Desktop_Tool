...
bool isEqual(svScalar a, svScalar b, bool highprecision){
  return fabs(a-b)<(highprecision?hepisode:lepisode);
}
...
{
   if((p[0]>lb[0]||fabs(p[0]-lb[0])<1e-9)
    &&(p[1]>lb[1]||fabs(p[1]-lb[1])<1e-9)
...
{
    svScalar length = (start[0]-end[0])*(start[0]-end[0])
                    + (start[1]-end[1])*(start[1]-end[1])
                    + (start[2]-end[2])*(start[2]-end[2]);
    length = length + radius*radius;
    length = sqrt(length)/2.;

    svVector3 p = (start + end);
    svScalar value = 2;
    p = p/value;

    float distance = PointtoLine2(rayP, rayD, p);
...
{
   if(fabs(v[0])<1e-9 && fabs(v[1])<1e-9 && fabs(v[2]-1)<1e-9)
   {
      box[0] = lb;
      box[1] = rb;
   }
   ...
   else if(fabs(v[0]-1)<1e-9 && fabs(v[1])<1e-9 && fabs(v[2])<1e-9)
   {
      box[0] = lb;
      box[1] = rb;
   }
   else
   {
      ...
      n[0]=0;n[1]=0;n[2]=1;
      inside[0]=LinetoPlane(lb,n,p1,v,p[0]);
      inside[1]=LinetoPlane(rb,n,p1,v,p[1]);
      n[0]=0;n[1]=1;n[2]=0;
      inside[2]=LinetoPlane(lb,n,p1,v,p[2]);
      inside[3]=LinetoPlane(rb,n,p1,v,p[3]);
      n[0]=1;n[1]=0;n[2]=0;
      inside[4]=LinetoPlane(lb,n,p1,v,p[4]);
      inside[5]=LinetoPlane(rb,n,p1,v,p[5]);
      ...
      for(int i=0;i<count;i++)
      {
        if(b[i][0]>r[0]) r[0] = b[i][0];
        if(b[i][1]>r[1]) r[1] = b[i][1];
        if(b[i][2]>r[2]) r[2] = b[i][2];
        if(b[i][0]<l[0]) l[0] = b[i][0];
        if(b[i][1]<l[1]) l[1] = b[i][1];
        if(b[i][2]<l[2]) l[2] = b[i][2];
      }
      box[0] = l;
      box[1] = r;
   }
...
svVector3 getProject(svVector3 p)
{
   ...
   gluProject(posX, posY, posZ, mvmatrix, projmatrix, viewport, &winX, &winY, &winZ);
   svVector3 pP;
   pP[0] = winX;
   pP[1] = winY;
   pP[2] = winZ;
...
{
    svVector3 pDir = cross(plane2-plane1, plane0-plane1);
    ...
    svVector3 x = rPos + rDir * (-GetDot(pDir, rPos - plane0) / nDotD);
    svVector3 a = plane0;
    svVector3 b = plane1;
    svVector3 c = plane2;
    svVector3 d = plane3;
...
bool rayBoxIntersect(svVector3 rPos, svVector3 rDir, svVector3 start, svVector3 end, svVector3 dir, svVector3 v, svScalar r)
{
    ...
    if(inside) return inside;
    inside = rayPlaneIntersect(rPos, rDir, p[5], p[1], p[3], p[7]);
    if(inside) return inside;
    inside = rayPlaneIntersect(rPos, rDir, p[0], p[5], p[1], p[4]);
    if(inside) return inside;
    inside = rayPlaneIntersect(rPos, rDir, p[2], p[7], p[3], p[6]);
    if(inside) return inside;
...
bool rayBoxIntersectD(svVector3 rPos, svVector3 rDir, svVector3 start, svVector3 end, svVector3 dir, svVector3 v, svScalar r, svScalar &distance)
{
    ...
    if(inside)
    {
         if(GetDot(inter - rPos, rDir)>0)
              d.push_back((rPos[0]-inter[0])*(rPos[0]-inter[0])
    ...
    inside = rayPlaneIntersect(rPos, rDir, p[0], p[5], p[7], p[2], inter);
    if(!flag) flag = inside;
    if(inside)
    {
         if(GetDot(inter - rPos, rDir)>0)
    ...
    inside = rayPlaneIntersect(rPos, rDir, p[5], p[1], p[3], p[7], inter);
    if(!flag) flag = inside;
    if(inside)
    {
         if(GetDot(inter - rPos, rDir)>0)
    ...
    inside = rayPlaneIntersect(rPos, rDir, p[0], p[5], p[1], p[4], inter);
    if(!flag) flag = inside;
    if(inside)
    {
         if(GetDot(inter - rPos, rDir)>0)
    ...
    inside = rayPlaneIntersect(rPos, rDir, p[2], p[7], p[3], p[6], inter);
    if(!flag) flag = inside;
    if(inside)
    {
         if(GetDot(inter - rPos, rDir)>0)
              d.push_back((rPos[0]-inter[0])*(rPos[0]-inter[0])
                   + (rPos[0]-inter[1])*(rPos[1]-inter[1])
                   + (rPos[2]-inter[2])*(rPos[2]-inter[2]));
    }
...
//    svVector3 x = rPos + rDir * (-GetDot(pDir, rPos - plane[0]) / nDotD);
//    svVector3 a = plane[0];
//    svVector3 b = plane[1];
//    svVector3 c = plane[2];
//    svVector3 d = plane[3];
...
//    if (GetDot(cross(a-d, x-d), pDir) < 0) return false;
    ...
    //if(GetDot(x-eye, rDir)<near) return false;

    distance = 0;//sqrt((x[0] - rPos[0])*(x[0] - rPos[0])
                //+  (x[1] - rPos[1])*(x[1] - rPos[1])
                //+  (x[2] - rPos[2])*(x[2] - rPos[2]));
 //   pos = x;
...
svInt  getNumOfIntegerDigits(svScalar num)
{
          ...
          if(fabs(numCopy - 0)<1e-20) multiplier = 0;
          else if(numCopy<1.)
          {
            while (numCopy<1.)//(fabs(numCopy-1.)>0.000001)&&(numCopy-1.)<0.000001)
                {
                       multiplier--;
                       numCopy *=10.;//cout<<std::setprecision(20)<<numCopy<<" "<<multiplier<<endl;
                       if(fabs(numCopy-1.)<1e-6)break;
                };
          }
          else if(numCopy==0)
                multiplier=0;
          else if(numCopy>1 ||fabs(num-1.)<1e-20)
          {
            while(1)
                {
...
//                glBegin(GL_QUAD_STRIP);
                for(int j=0;j<=white_segment;j++)
                {
                        ...
                        cylinder_seg_pos[count] = rotate_base;
...
*/
        ...
        svVector3 head = glyph + radius * v;
        svVector3 top = glyph + height* dir;
//        cerr<<height<<" "<<dir[0]<<" "<<dir[1]<<" "<<dir[2]<<endl;
        int count = 0;
        vector<svVector3> base;
        for(int i=0;i<num;i++)
        {
...
//                glBegin(GL_QUAD_STRIP);
                for(int j=0;j<=white_segment;j++)
                {
                        ...
                        cylinder_seg_pos[count] = rotate_base;
                        cylinder_seg_norm[count] = rotate_normal;
                        cylinder_seg_pos[count+1] = rotate_top;
                        cylinder_seg_norm[count+1] = rotate_normal;
//                        glNormal3f(rotate_normal[0], rotate_normal[1], rotate_normal[2]);
//                        glVertex3f(rotate_base[0], rotate_base[1], rotate_base[2]);
//                        glNormal3f(rotate_normal[0], rotate_normal[1], rotate_normal[2]);
//                        glVertex3f(rotate_top[0], rotate_top[1], rotate_top[2]);
...
//                glBegin(GL_QUAD_STRIP);
                for(int j=0;j<=white_segment;j++)
                {
                        ...
                        cone_seg_pos[count] = rotate_base;
...
void RenderFan(ViewProperty &property, svVector3 glyph, svVector3 dir,
                svScalar radius, svScalar height,
  ...
  svVector3 p1, p2;
  p1[0] = glyph[0] * property.tm[0]
        + glyph[1] * property.tm[4]
        + glyph[2] * property.tm[8]
        + property.tm[12];
  p1[1] = glyph[0] * property.tm[1]
        + glyph[1] * property.tm[5]
        + glyph[2] * property.tm[9]
        + property.tm[13];
  p1[2] = glyph[0] * property.tm[2]
        + glyph[1] * property.tm[6]
        + glyph[2] * property.tm[10]
        + property.tm[14];
  svVector3 end = glyph + dir * height;
  p2[0] = end[0] * property.tm[0]
        + end[1] * property.tm[4]
        + end[2] * property.tm[8]
        + property.tm[12];
  p2[1] = end[0] * property.tm[1]
        + end[1] * property.tm[5]
        + end[2] * property.tm[9]
        + property.tm[13];
  p2[2] = end[0] * property.tm[2]
        + end[1] * property.tm[6]
        + end[2] * property.tm[10]
        + property.tm[14];
  svVector3 newdir = normalize(p2-p1);
  svVector3 eye=p1;eye[2] = property.eye[2];
  svVector3 eyedir = normalize(eye-p1);
  svVector3 v = normalize(cross(newdir, eyedir));
  svVector3 pv = p1 + radius * v;
  svVector3 newv;
  newv[0] = pv[0] * invert_tb[0]
        + pv[1] * invert_tb[4]
        + pv[2] * invert_tb[8]
        + invert_tb[12];
  newv[1] = pv[0] * invert_tb[1]
        + pv[1] * invert_tb[5]
        + pv[2] * invert_tb[9]
        + invert_tb[13];
  newv[2] = pv[0] * invert_tb[2]
        + pv[1] * invert_tb[6]
        + pv[2] * invert_tb[10]
        + invert_tb[14];
  newv = normalize(newv-glyph);
  svVector3 newn = normalize(cross(newv, dir));
  glEnable(GL_LIGHTING);
  glBegin(GL_POLYGON);
  glNormal3f(newn[0], newn[1], newn[2]);
  glVertex3f(glyph[0], glyph[1], glyph[2]);
  svVector3 head = glyph + radius * newv + height*dir;
  for(int i=0;i<=segment;i++){
