...
namespace __svl_lib {
svLinearArrow::svLinearArrow(svQDOTData *d):svArrowGlyph(d)//svVectorField* f, int numPlane)
{
  //field = f;
   ...
   //cerr<<"arrow "<<count<<" "<<index<<" "<<arrow_vertices_size<<" "<<arrow_indices_size<<" "<<arrow_base_vertices_size<<endl;
    ...
    for(int i =0;i<seed_num;i++)
    {
                for(int j=0;j<myData->splitData[i].size();j++)
                {
                        ...
                        svScalar scale =d.den*glyphScale;
...
void svLinearArrow::GenerateTubes()
{
    int count1 = 0;
    int count2 = 0;
    for(int i =0;i<seed_num;i++)
...
void svLinearArrow::GenerateIndex()
{
    int index1 = 0;
    int count1 = 0;
    int index2 = 0;
