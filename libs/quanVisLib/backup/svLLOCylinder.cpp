...
svLLOCylinder::svLLOCylinder(svVectorField *f) : svCylinderGlyph(f)
{

}
void svLLOCylinder::RenderSample()
{
...
void svLLOCylinder::GenerateTubes()
{
    int count1 = 0;
    int count2 = 0;
    for(int i =0;i<seed_num;i++)
    {
                ...
                {
                        double radius = glyphScale*(exp[i][j]+expScale);//glyphTubeRadius;
                        svVector3 end;
                        if((exp[i][j]+expScale)<1) radius = glyphScale/10.;
                        svScalar scale = coe[i][j]*glyphScale;
                        svVector3 tube_seg_norm[(CYLINDERSLICE+1)*2];
                        svVector3 tube_seg_pos[(CYLINDERSLICE+1)*2];

                        //cerr<<radius<<" "<<scale<<" "<<coe[i][j]<<" "<<exp[i][j]<<" "<<expScale<<endl;
...
void svLLOCylinder::GenerateIndex()
{  
...
void svLLOCylinder::GenerateLegend()
{
    int count = 0;
    for(int i =0;i<seed_num;i++)
    {
...
void svLLOCylinder::RenderLegend()
{
