...
#include "Utility.h"
	...
	template <class T>
	static inline int Utility::findNEleSmallerInSortedList(const vector<T> &sortedList, const T val){
		int index;
	...
	template<class T>
	static inline void Utility::saveTriangulationCurveFile(const char* filename, const vector<vector<vector<T> > > & Vs){
		int nV = 0;
	...
	static void Utility::flipSufOrientation(vector<vector<int> > & sufF){
		int tmpV = -1;
	...
	static void UtilityforceSufOriented(vector<vector<int> > & sufF){
	...
	template <class T>
	static void Utility::writeOutOneVector(const char* filename, T &vec); 
	...
	static void Utility::findOrderedPairs(vector<vector<int> > &in_Pairs, vector<int> &in_PairsInd, int in_startPairID, int in_startEleID, vector<int>& out_PairIDs, vector<int>&out_EleIDs){
	...
	static void Utility::findMergingOfTwoLists(vector<float> &list1, vector<float> &list2, vector<vector<int> > &merge1, vector<vector<int> > &merge2, float err){
		int n1 = list1.size();
		int n2 = list2.size();
		int i1=0, i2=0;
		merge1.resize(n1+1);
	...
	static inline void Utility::readSkipNWord(ifstream &ifs, int n){
		string word;
		for(int i=0; i<n; ++i){
			ifs >> word;
		}
	...
	template <class T>
	static void Utility::read2DArrayVector(const char* filename, vector<vector<T> > &data){
...
template <class T>
void writeVector(vector<T>& vec, ofstream& ofs){
