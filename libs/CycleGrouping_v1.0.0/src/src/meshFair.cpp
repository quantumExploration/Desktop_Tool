...
void LuMesh::InputData( const std::vector<float> &mver,const intvector &mface, 
...
void LuMesh::OutputData( std::vector<float> &mver, intvector &mface)
{
	mver.clear();
	mface.clear();
	int ind = 0;
...
{
	// cout << "In JuFair!" << endl;
	// cout << "ratio:" << ratio << "times:" << times << endl;
	// cout<<"smoothing....\t";
	//initialization
	double* oldver;
	double* newver;
...
void LuMesh::LaplacianSmooth(float ratio, int times)
{
	//initialization
	double* oldver;
	double* newver;
	...
	oldver = new double[ sufvernum*3 ];
	newver = new double[ sufvernum*3 ];
	firstdiffer = new double[sufvernum*3];
...
void inline LuMesh::SDUmbraFair(double* oldver, double* newver,  double* oldfirstdiffer, double* newfirstdiffer, 
							  double* seconddiffer, vector<int>&vermark, vector<intvector>& verneighbr,
							  HashMap& ver2edgehash, vector<int>&edgelist, vector<double>&wlist)
{
	//	vector<float> wlist;
	...
	int curver;
	for( int i = 0; i < selectVerList.size(); i ++)
	{
		cout<<"-------------------------------------------------"<<endl;
		...
		cout<<"neighbr:"<<endl;
		for( int j = 0; j < verneighbr[ curver ].size(); j++)
		{
...
void LuMesh::SDUmbraFair(int times)
{
	cout<<"smoothing....\t";
	//initialization
	double* oldver;
