...
//"/home/davinci/Documents/henan/code/dual_depth_peeling/OpenGL/src/dual_depth_peeling/shaders/"
//"src/dual_depth_peeling/shaders/"
...
void LoadModel(const char *model_filename)
{
	...
	g_bbScale = 1.0 / nv::length(diag) * 1.5;
...
void DrawModel()
{
	...
	glNormalPointer(GL_FLOAT, stride, (GLubyte *)NULL + normalOffset);
        //glColorPointer(g_model->getPositionSize(), GL_FLOAT, stride, NULL);
        //glColorPointer(g_model->getNormalSize(),stride, (GLubyte *)NULL + normalOffset);
...
void RenderDualPeeling()
{
	...
	for (int pass = 1; g_useOQ || pass < g_numPasses; pass++) {
		...
		DrawModel();
        float tmpa = 0.5;
                g_shaderDualPeel.setUniform("Alpha", (float*)&tmpa, 1);
                glColor4f(0,0,1,0.5);
                glutSolidSphere(0.1,10,10);
...
void RenderAverageColors()
{
	...
	DrawModel();
       ...
       float tmp = 0.5;
        g_shaderAverageInit.setUniform("Alpha", (float*)&tmp, 1);
        glColor4f(0,0,1,0.5);
        glPushMatrix();
