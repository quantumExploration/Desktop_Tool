...
void HeightField::PrintValidValues(char *fname)
{
     ...
     for (int whichArc = 0; whichArc < nValidArcs; whichArc++)
     { 
	int arc = valid[whichArc]; 
        long topNode = superarcs[arc].topID, bottomNode = superarcs[arc].bottomID;
        outfile<<topNode<<" "<<bottomNode<<" ";

/*        if(supernodes[topNode].IsLowerLeaf())
        {
          long theArc = supernodes[topNode].downList;
          outfile<<superarcs[theArc].nodesThisSideOfTop<<" ";
        }
        else if(supernodes[topNode].IsUpperLeaf())
        {
          long theArc = supernodes[topNode].upList;
          outfile<<superarcs[theArc].nodesThisSideOfBottom<<" ";
        }
        else 
          outfile<<-1<<" ";
        if(supernodes[bottomNode].IsLowerLeaf())
        {    
          long theArc = supernodes[bottomNode].downList;
          outfile<<superarcs[theArc].nodesThisSideOfTop<<" ";
        }
        else if(supernodes[bottomNode].IsUpperLeaf())
        {
          long theArc = supernodes[bottomNode].upList;
          outfile<<superarcs[theArc].nodesThisSideOfBottom<<" ";
        }
        else  
          outfile<<-1<<" ";
  */
       // outfile<<superarcs[arc].nodesOnArc<<" ";//<<nVertices<<" ";
       // outfile<<superarcs[arc].nodesThisSideOfTop<<" "<<superarcs[arc].nodesThisSideOfBottom<<" ";      
        outfile<<setprecision(16)<<*(supernodes[topNode].value)
               <<" "<<*(supernodes[bottomNode].value)<<" ";
        outfile<<superarcs[arc].nodesOnArc<<endl;
//        printf("id %d location %d %d %f %f %f %f %e %e\n",arc,topNode, bottomNode, supernodes[topNode].xPosn,supernodes[topNode].yPosn, supernodes[bottomNode].xPosn,supernodes[bottomNode].yPosn, *(supernodes[topNode].value),*(supernodes[bottomNode].value));
