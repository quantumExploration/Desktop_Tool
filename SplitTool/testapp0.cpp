...
float alpha;
int g_numPasses = 4;
int g_imageWidth=1800;
int g_imageHeight=1000;
...
void RenderDualPeeling()
{
        ...
        int currId = 0;
//
        for (int pass = 1; g_useOQ || pass < g_numPasses ; pass++) {//g_useOQ ||
                currId = pass % 2;
                cerr<<pass<<endl;
                int prevId = 1 - currId;
                int bufId = currId * 3;
...
int main(int argc, char *argv[])
{
  ...
  eye[1]=0;
  eye[2]=100;
  image_width = g_imageWidth;
  image_height = g_imageHeight;
	glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA | GLUT_DEPTH | GLUT_MULTISAMPLE);
	glutInitWindowSize(g_imageWidth, g_imageHeight);
  glutInitWindowPosition(0,0);
  glutInit(&argc, argv);
	glutCreateWindow("Tool");
  if (glewInit() != GLEW_OK)
  {
