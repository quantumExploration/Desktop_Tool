...
void readConfig(char *inputfile){
  ...
  paser->SetPlane(myData->getPlaneD(), c);
...
void UpdateVisible(){
  myData->testvis(contour_layer, contour_index, lb, rb);cerr<<contour_layer<<" "<<contour_index<<endl;
  UpdateVis();
}
void UpdateSingleContour(){
  //update paser: contour property
  //
}
  ...
  UpdateImage();
cerr<<"image"<<endl;
  myData->setSingleContour(paser->GetContourTreeDir(),
                            paser->GetPlaneName(),
...
{  
  ...
  image = new svContourImageDrawing(myData->getRaw());

  cerr<<"Render routine..."<<endl;
  if(!paser->GetIsContour())
     lt = new svLTArrow(myData->getRaw());
  else
  {//cout<<"Contour"<<endl;
   lt = new svLTArrow(myData->getContour());
  } 
  lt->SetColorByClusterMag();//(blackcolor);
  lt->UpdateData();cerr<<"update data"<<endl;
  lt->SetES(myData->getScaling());
  lt->SetNumPower(myData->getNumPower());
  lt->SetScaling(myData->getScaling());
  lt->UpdateIndex();cerr<<"update index"<<endl;
  ...
  lc = new svLCArrow(myData->getRaw());
  lc->SetES(myData->getScaling());
  lc->SetNumPower(myData->getNumPower());
  lc->SetScaling(myData->getScaling());
