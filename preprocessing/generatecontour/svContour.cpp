...
void svContour::Get2DContourLine(char *infile,  double density) //density is only the ratio
{
//#ifdef _VTK
        vtkUnstructuredGridReader *pl3d = vtkUnstructuredGridReader::New();
        pl3d->SetFileName(infile);
	...
	vector<double> p3;
        ...
        for(int i=0;i<pd->GetNumberOfCells();i++)
        {
                ...
                cell->GetPoints()->GetPoint(0,p1);
                cell->GetPoints()->GetPoint(1,p2);
...
void svContour::SavePointtoSVL(char *file)
{
        ofstream ofile;
        ofile.open(file);

        ofile<<pointnum/2<<endl;

        for(int i=0;i<pointnum;)
        {
           int ii = i;
           ofile<<2<<endl;
           /*ofile<<isoPointPos[ii][0]<<" "
                <<isoPointPos[ii][1]<<" "
                <<isoPointPos[ii][2]<<" "
                <<isoPointDir[ii][0]<<" "
                <<isoPointDir[ii][1]<<" "
                <<isoPointDir[ii][2]<<" "
                <<isoPointDen[ii]<<endl;
           ofile<<isoPointPos[ii+1][0]<<" "
                <<isoPointPos[ii+1][1]<<" "
                <<isoPointPos[ii+1][2]<<" "
                <<isoPointDir[ii+1][0]<<" "
                <<isoPointDir[ii+1][1]<<" "
                <<isoPointDir[ii+1][2]<<" "
                <<isoPointDen[ii+1]<<endl;
*/
           ofile<<isoPointPos[ii][0]<<" "<<isoPointPos[ii][1]<<" "<<isoPointPos[ii][2]<<endl;
           ofile<<isoPointPos[ii+1][0]<<" "<<isoPointPos[ii+1][1]<<" "<<isoPointPos[ii+1][2]<<endl;
